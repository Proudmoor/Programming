...
int Shell_run(apr_pool_t *p, Shell *cmd){
	...
	rv = apr_procattr_cmdtype_set(attr, APR_PROGAM_PATH);
	check(rv == APR_SUCCESS, "Failed to set cmd type.");

	rv = apr_proc_create(&newproc, cmd->exe, cmd->args, NULL, attr, p);
	check(rv == APR_SUCCESS, "Failed to run command.");

	rv = apr_pro_wait(&newproc, &cmd->exit_code, &cmd->exit_why, APR_WAIT);
...
Shell CLEANUP_SH ={
	.exe = "rm",
	.dir = "/tmp",
	.args = {"rm", "-rf", "/tmp/pkg_build", "/tmp/pkg_src.tar.gz",
